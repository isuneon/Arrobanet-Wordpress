 ...
 * @return array
 */
function bitunit_lite_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! bitunit_lite_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'bitunit_lite_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'bitunit_lite_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'bitunit_lite' ) . $network['name'],
